...
class ExploreController extends GetxController {
  ...
  Future<void> _fetchHotelsForSelectedCategories() async {
    ...
    try {
      final selectedCities = selectedCategories
  ...
  List<Hotel> get filteredHotels {
    ...
    if (selectedCategories.isNotEmpty) {
      final selectedCities = selectedCategories
          ...
          .toSet();
      result = result
          .where((hotel) => selectedCities.contains(hotel.city.toLowerCase()))
          .toList();
    }
    ...
    if (query.isNotEmpty) {
      result = result.where((hotel) {
        ...
        final addressMatch =
            hotel.address?.toLowerCase().contains(query) ?? false;
  ...
  void fetchCities() {
    ...
    debugPrint(
      "Cities: ${cities.value.map((city) {
        print("URL: " + city.imageURL + "\n Name: " + city.name);
      })}",
    );
  }
